...
import { perfumersByLetter } from "@/data/constants";
...
const Perfumers = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const sections = perfumersByLetter.map((opt) =>
      ...
      for (let i = 0; i < sections.length; i++) {
        ...
        if (section) {
          ...
          if (rect.top <= headerHeight + 10 && rect.bottom > headerHeight) {
            setStage({
              type: perfumersByLetter[i].name,
              des: perfumersByLetter[i].des,
            });
            foundSection = true;
            break;
          }
      ...
      if (!foundSection && window.scrollY < 100) {
        setStage({
          type: perfumersByLetter[0].name,
          des: perfumersByLetter[0].des,
        });
      }
    };
    ...
    <div className="flex flex-col gap-[5rem]">
      ...
      <div className="fixed top-16 left-0 right-0 z-40 bg-white border-b border-gray-200 flex items-center justify-between px-10 py-4">
        <div className="flex flex-col relative mt-2" ref={dropdownRef}>
          ...
          >
            {perfumersByLetter.map((option, idx) => (
              <li
                key={idx}
      ...
      <div className="px-6 mt-28 pb-10">
        <div className="flex flex-col gap-[5rem]">
          {perfumersByLetter.map((option, index) => {
            const sectionId = `section-${option.name
              .toLowerCase()
              .replace(/[^a-z0-9]+/g, "-")}`;
            return (
