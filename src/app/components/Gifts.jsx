          ...
          <div className="flex flex-col gap-5 items-center">
            ...
            <div className="grid grid-cols-2 xs:grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4  gap-4">
              {giftCategories.map((gift, idx) => (
                <Link href={`${gift.link}`} key={idx} className="flex flex-col gap-3 p-2 md:p-4 group">
                  {/* Image Container */}
                  <div className="relative w-full dark:bg-white aspect-square
                 overflow-hidden">
                    <img
                      src={gift.img}
                      alt={gift.name}
                  ...
                  <div className="flex flex-col gap-2">
                    <h1 className="text-xs sm:text-sm transition-all
